    ...
    try {
      ...
      if (response.status === 200) {
        ...
        if (response2?.status === 200) {
          ...
          setSuccessDialog({
            isOpen: true,
            message: `Student ${val.LRN} has been archived!`,
          });
        }
      }
      setLoadingDialog({ isOpen: false });
    } catch (error) {
      console.log("🚀 ~ file: Student.jsx:241 ~ toggleStatus ~ error", error);
      setLoadingDialog({ isOpen: false });
      if (!error?.response) {
        setErrorDialog({
          isOpen: true,
                ...
                onClick={() => {
                  setValidateDialog({
                    ...
                    onConfirm: () => {
                      setConfirmDialog({
                        isOpen: true,
                        title: `Are you sure to archive student ${params?.row?.LRN}`,
                        onConfirm: () => {
